...
from models.modeling_fuse_cub_step1_v10 import VisionTransformer_TRT, CONFIGS
from utils.scheduler import WarmupLinearSchedule, WarmupCosineSchedule
from utils.data_utils import get_loader
from utils.dist_util import get_world_size
from utils.checkpoints_w11 import check_load_fuse
...
def save_model(args, model, global_step, acs):
    ...
    model_checkpoint = os.path.join(args.output_dir, "checkpoint_cub_step1_v10_ablation1_nomse_" + str(global_step) + '_' + str(acs[0].item())[0:7] + '_' + str(acs[1].item())[0:7] + '_' + str(acs[2].item())[0:7] + ".pth")
    if args.fp16:
        checkpoint = {
...
def valid(args, model, test_loader, global_step):
    ...
    model.eval()
    TFG_preds, TRT_preds, CAM_preds, GCN_preds, all_preds, all_label = [], [], [], [], [], []
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        try:
            with torch.no_grad():
                TRT_logits, CAM_logits, loss_loc = model(x, pathi)

                eval_loss = torch.nn.CrossEntropyLoss()(TRT_logits, y) + torch.nn.CrossEntropyLoss()(CAM_logits, y) + loss_loc
                eval_loss = eval_loss.mean()
                eval_losses.update(eval_loss.item())

                preds_TRT = torch.argmax(TRT_logits, dim=-1)
                preds_CAM = torch.argmax(CAM_logits, dim=-1)
                preds_all = torch.argmax(TRT_logits + CAM_logits, dim=-1)

            if len(all_label) == 0:
                TRT_preds.append(preds_TRT.detach().cpu().numpy())
                CAM_preds.append(preds_CAM.detach().cpu().numpy())
                all_preds.append(preds_all.detach().cpu().numpy())
                all_label.append(y.detach().cpu().numpy())
            else:
                TRT_preds[0] = np.append(
                    TRT_preds[0], preds_TRT.detach().cpu().numpy(), axis=0
                )
                CAM_preds[0] = np.append(
                    CAM_preds[0], preds_CAM.detach().cpu().numpy(), axis=0
                )
                all_preds[0] = np.append(
                    all_preds[0], preds_all.detach().cpu().numpy(), axis=0
                )
    ...
    accuracy_all = simple_accuracy(all_preds, all_label)
    accuracy_all = torch.tensor(accuracy_all).to(args.device)

    logger.info("\n")
    logger.info("Validation Results")
    logger.info("Global Steps: %d" % global_step)
    logger.info("Valid Loss: %2.5f" % eval_losses.avg)
    logger.info("Valid Accuracy TRT: %2.5f, CAM: %2.5f, acc: %2.5f" % (accuracy_TRT, accuracy_CAM, accuracy_all))

    return [accuracy_TRT, accuracy_CAM, accuracy_all]
...
def train(args, model):
    ...
    while True:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            try:
            #if True:
                TRT_logits, CAM_logits, loss_loc = model(x, pathi, y)
                loss_trt = torch.nn.CrossEntropyLoss()(TRT_logits, y)
                loss_cam = torch.nn.CrossEntropyLoss()(CAM_logits, y)
                loss = loss_trt + loss_cam + loss_loc
                loss = loss.mean()

                preds_TRT = torch.argmax(TRT_logits, dim=-1)
                preds_CAM = torch.argmax(CAM_logits, dim=-1)
                preds_all = torch.argmax(TRT_logits + TRT_logits, dim=-1)
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    epoch_iterator.set_description(
                        "Training (%d/%d Steps) loss=%2.5f(trt=%2.5f, cam=%2.5f, loc=%2.5f)" % (global_step, t_total, losses.val, losses_trt.val, losses_cam.val, losses_loc.val)
                    )
        ...
        logger.info("train acc_TRT: %2.3f, acc_CAM: %2.3f, acc: %2.3f" % (accuracy_TRT, accuracy_CAM, accuracy))
        losses.reset()
        if global_step % t_total == 0:
            break
...
def main():
    ...
    #parser.add_argument("--pretrained_model", type=str, default='./outputs/cub_200_2011_v10/checkpoint_cub_step1_v10_ablation1_nomse_111000_0.90231_0.90265_0.90490.pth',
    ...
    parser.add_argument("--num_steps", default=1000000*4, type=int,
