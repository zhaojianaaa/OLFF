...
def check_load(model, pretrained_model):
    ...
    for k, v in model_dict.items():
        if k in checkpoint and checkpoint[k].shape != model_dict[k].shape:
            ...
            checkpoint[k] = checkpoint[k.replace('score_layer','layer.10')]
            print(k + ' copy from ' + k.replace('score_layer','layer.10'))
...
def check_load_fuse(model, pretrained_model):
    ...
    for k, v in model_dict.items():
        if k in checkpoint and checkpoint[k].shape != model_dict[k].shape:
            ...
            checkpoint[k] = checkpoint[k.replace('camfuse_block.cam_tr','layer.10')]
            print(k + ' copy from ' + k.replace('camfuse_block.cam_tr','layer.10'))
        elif k not in checkpoint and 'score_norm' in k:
            checkpoint[k] = checkpoint[k.replace('score_norm','part_norm')]
