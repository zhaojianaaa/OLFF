...
def check_load_fuse(model, pretrained_model):
    ...
    for k, v in model_dict.items():
        ...
        #elif k not in checkpoint and 'trt_norm' in k:
        #    checkpoint[k] = checkpoint[k.replace('trt_norm','transformer.encoder.part_norm')]
        #    print(k + ' copy from ' + k.replace('trt_norm','transformer.encoder.part_norm'))
        elif k not in checkpoint and 'part_norm' in k:
            checkpoint[k] = checkpoint[k.replace('part_norm','transformer.encoder.part_norm')]
            print(k + ' copy from ' + k.replace('part_norm','transformer.encoder.part_norm'))
        #elif k not in checkpoint and 'trt_head' in k:
        #    checkpoint[k] = checkpoint[k.replace('trt_head','part_head')]
        #    print(k + ' copy from ' + k.replace('trt_head','part_head'))
